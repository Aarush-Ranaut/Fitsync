...
import 'dart:ui';
import '../widgets/home_screen.dart'; // Import the HomeScreen
import '../widgets/user_info/height_picker_page.dart'; // Import HeightPickerPage
import '../widgets/user_info/weight_picker_page.dart';
import 'user_info/gender_selection.dart'; // Import WeightPickerPage
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  final TextEditingController _firstNameController = TextEditingController();
  final TextEditingController _lastNameController = TextEditingController();
  int? _height; // Store height as an integer
  double? _weight; // Store weight as a double
  final TextEditingController _ageController = TextEditingController();
  DateTime? _birthDate; // To store the selected birth date
  ...
  Future<void> _fetchUserData() async {
    try {
      ...
      if (userDoc.exists) {
        ...
        _lastNameController.text = data['lastName'] ?? '';
        setState(() {
          _height = data['height']?.toInt() ?? 170; // Default to 170 cm
          _weight = data['weight']?.toDouble() ?? 70.0; // Default to 70.0 kg
  ...
  Future<void> _navigateToHeightPicker() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const HeightPickerPage(isFromEditProfile: true),
      ),
    );
    if (result != null) {
      setState(() {
        _height = result as int;
      });
    }
  }

  Future<void> _navigateToWeightPicker() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const WeightPickerPage(isFromEditProfile: true),
      ),
    );
    if (result != null) {
      setState(() {
        _weight = (result as double);
      });
    }
  }

  Future<void> _navigateToGenderSelection() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) =>
            const GenderSelectionScreen(isFromEditProfile: true),
      ),
    );
    if (result != null) {
      setState(() {
        _selectedGender = result as String;
      });
    }
  }
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        'Weight: ${_weight != null ? _weight!.toStringAsFixed(1) : "Select"} kg',
                        style: const TextStyle(
                            color: Colors.white70, fontSize: 16),
                      ),
                      const Icon(Icons.arrow_forward_ios,
